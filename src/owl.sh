...
# Resolve relative to this file (not $0) so tests that source owl.sh from a
# different directory still find the helper.
# shellcheck disable=SC1091
. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/lib/deterministic_tests.sh"
# shellcheck disable=SC1091
. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/lib/plan_parsing.sh"
