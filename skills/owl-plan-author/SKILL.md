...
**Worktree note:** Owl executes plans inside a temporary git worktree, NOT in the original repo directory. The agent is launched with `cd` into the worktree root (e.g., `.work/worktrees/<plan-name>/saudade/`). This means:
- **Use relative paths or repo-name-based paths for instructions** (e.g., "in `pipeline/foo.py`"), not absolute paths like `/Users/lanabarreto/Documents/Murilo/saudade/pipeline/foo.py`.
