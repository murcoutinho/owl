...
Example chain: plan `019` touches `project-api`. Plan `020` touches both `project-api`
and `project-web`, and the `project-api` half depends on plan `019`. Add
`base-branch: owl/019-...` to plan `020`. When owl runs `020`, the `project-api`
repo checks out `019`'s branch and the new commits stack on top; the
`project-web` repo has no `019` branch on origin and silently uses `main`.
...
cp /Users/user/path/to/projects/owl/skills/owl-plan-author/SKILL.md ~/.claude/skills/owl-plan-author/SKILL.md
```
