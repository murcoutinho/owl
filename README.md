...
3. Every 10 minutes, the agent picks up the next plan by order and:
   - **Executes** the plan via Claude Code (Sonnet 4.6)
   - **Commits** all changes across any git repos in the parent directory
   - **Reviews** the commit in parallel (Codex + Claude Code)
   - **Fixes** issues from the review via Claude Code
...
- Uses `claude --dangerously-skip-permissions` — review plans before dropping them in
